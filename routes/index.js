...
router.get('/medicine', function(req, res, next) {
  res.render('medicine',{user:req.user});
});
...
router.get('/doctor', function(req, res, next) {
  res.render('doctor',{user:req.user});
});
...
var addans=mongoose.model('addans',forumans);
//for User
...
router.post('/addquestion',function(req,res){
	...
	tags:req.body.tags
		...
		{
	//		res.location('forum');	
			return res.render('/forum',{user:req.user});
		}
	});
});
//add answer
...
router.post('/addanswer',function(req,res){
	...
	}).save(function(err,adocs){
		if(err){
      console.log(err);
			res.send("plz try after some time");}
		else 
		{
	//		res.location('forum');
...
router.post('/User', function(req, res) {
  ...
  user.save(function(err) {
    req.logIn(user, function(err,success) {
      req.flash('success','Hello '+ user.username +' signup successfully');
      res.redirect('/');
    });
  });
});
